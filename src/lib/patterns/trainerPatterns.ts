...
function calculateScore(
  winRate: number,
  starts: number
): { score: number; tier: TrainerProfile['tier'] } {
 ...
 */
export function extractTrainerPatternsFromHorse(
  horse: HorseEntry
): Map<string, TrainerPatternStats> {
  ...
  patterns.set(overallKey, createPatternStats(trainerName, {}, `${horse.trainerName} overall`));
  ...
  try {
    for (const pp of horse.pastPerformances) {
      ...
      if (!patterns.has(surfaceKey)) {
        patterns.set(
          surfaceKey,
          createPatternStats(trainerName, { surface: pp.surface }, `On ${pp.surface}`)
        );
      }
 ...
 */
export function buildTrainerProfile(trainerName: string, horses: HorseEntry[]): TrainerProfile {
