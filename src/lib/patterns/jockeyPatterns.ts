 ...
 */
export function extractJockeyPatternsFromHorses(
  horses: HorseEntry[]
): Map<string, JockeyProfile> {
    ...
    for (const horse of horses) {
      ...
      for (const pp of horse.pastPerformances) {
        ...
        if (!profile.byTrack.has(pp.track)) {
          profile.byTrack.set(
            pp.track,
            createJockeyPatternStats(pp.jockey, { trackCode: pp.track }, `At ${pp.trackName || pp.track}`)
          )
        }
        ...
        if (runningStyle !== 'unknown') {
          ...
          updateJockeyPatternStats(profile.byRunningStyle.get(runningStyle)!, pp.finishPosition, beyer)
        }
        ...
        updateJockeyPatternStats(profile.byDistanceCategory.get(distCat)!, pp.finishPosition, beyer)
      }
    }
 ...
 */
export function buildJockeyProfile(
  jockeyName: string,
  horses: HorseEntry[]
): JockeyProfile {
...
export function calculateJockeyPatternScore(
  ...
  try {
    ...
    const horseRunningStyle = horse.pastPerformances.length > 0
