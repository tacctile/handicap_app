...
export function calculateDutchBook(config: DutchConfig): DutchResult {
  ...
  const sanitizedHorses = horses.map(sanitizeHorse).filter((h): h is DutchHorse => h !== null);
  ...
  if (sanitizedHorses.length > RECOMMENDED_MAX_HORSES) {
    warnings.push(`Consider using ${RECOMMENDED_MAX_HORSES} or fewer horses for optimal value`);
  }
