...
function generateTier1Bets(
  ...
  if (tier1Horses.length >= 2) {
    ...
    bets.push({
      ...
      confidence: Math.round(
        boxHorses.reduce((sum, h) => sum + h.confidence, 0) / boxHorses.length
      ),
  ...
  const otherContenders = allHorses
  ...
  if (tier1Horses.length >= 3) {
    ...
    bets.push({
      ...
      confidence:
        Math.round(triHorses.reduce((sum, h) => sum + h.confidence, 0) / triHorses.length) - 15,
      icon: BET_TYPE_CONFIG.trifecta_box.icon,
    });
  } else if (tier1Horses.length >= 2) {
    // Use tier 1 + tier 2 horses for trifecta
    const triHorses = [
      ...tier1Horses.slice(0, 2),
      ...allHorses.filter((h) => !tier1Horses.includes(h)).slice(0, 1),
    ];
...
function generateTier2Bets(
  ...
  if (tier1Horses.length > 0) {
    ...
    bets.push({
      ...
      horseNumbers: [
        topValueHorse.horse.programNumber,
        ...chalkHorses.map((h) => h.horse.programNumber),
      ],
      amount: keyAmount,
      totalCost: keyCost,
...
function generateTier3Bets(
  ...
  const otherHorses = allHorses.filter((h) => h.horseIndex !== bombHorse.horseIndex).slice(0, 5);
  ...
  if (allHorses.length >= 4) {
    const superHorses = [
      bombHorse,
      ...allHorses.filter((h) => h.horseIndex !== bombHorse.horseIndex).slice(0, 3),
    ];
    const superAmount = 0.1;
    const superCost = 2.4; // $0.10 box of 4
    bets.push({
      type: 'superfecta',
