...
function createMockRaceHeader(): RaceHeader {
  return {
    ...
    distance: '6F',
    surface: 'dirt',
    raceClassification: { type: 'claiming', purse: 25000 },
    fieldSize: 8,
    purse: 25000,
  } as RaceHeader
}
...
function createMockBankroll(overrides: Partial<UseBankrollReturn> = {}): UseBankrollReturn {
  ...
  return {
    ...
    getBetAmount: (confidence: number, tierMultiplier?: number) => Math.round(10 * (tierMultiplier || 1)),
    getUnitMultiplier: () => 1,
...
describe('generateWindowInstruction', () => {
  const mockHorse = (programNumber: number) => ({
    ...
    overlay: {
      ...
      evPerDollar: 0.25,
      evPercent: 25,
      classification: 'overlay' as const,
      valueClassification: 'overlay',
