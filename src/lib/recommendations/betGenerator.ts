...
function generateTier1Bets(
  ...
  const baseAmount = calculateBetAmount(
    topHorse.confidence,
    'tier1',
    bankroll
  )
...
function generateTier3Bets(
  ...
  if (allClassifiedHorses.length >= 4) {
    const superHorses = [bombHorse, ...allClassifiedHorses.filter(h => h.horseIndex !== bombHorse.horseIndex).slice(0, 3)]
    bets.push(createGeneratedBet({
      type: 'superfecta',
      typeName: 'Superfecta',
...
function generateValueBombBets(
  ...
  const valuePlays = allClassifiedHorses.filter(h =>
    h.overlay.isPositiveEV &&
    h.overlay.overlayPercent >= 50 &&
    h.odds >= 5 // At least 5-1 odds
  )
  ...
  for (const horse of valuePlays.slice(0, 2)) {
    const amount = Math.max(
      ...
      Math.min(
        ...
        Math.round(calculateBetAmount(horse.confidence, 'tier2', bankroll) * (horse.overlay.evPerDollar + 1))
      )
    )
    ...
    if (mode === 'simple') {
      switch (style) {
        case 'safe':
          ...
          isRecommended = bet.tier === 'tier1' ||
            (bet.tier === 'tier2' && bet.confidence >= 65)
          break
        case 'balanced':
...
export function generateRecommendations(input: GeneratorInput): BetGeneratorResult {
  ...
  try {
    ...
    const tier3Bets = tier3Group
      ? generateTier3Bets(tier3Group, allClassifiedHorses, nuclearLongshotsMap, raceNumber, bankroll)
      : []
    ...
    const diamondBets = generateDiamondBets(
      validatedDiamonds,
      allClassifiedHorses,
      raceNumber
    )
    const valueBombBets = generateValueBombBets(
      allClassifiedHorses,
      raceNumber,
      bankroll
    )
