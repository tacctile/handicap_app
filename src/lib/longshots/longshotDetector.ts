...
export function detectPaceDevastation(
  ...
  try {
    ...
    const closingWins = runningStyle.evidence.filter(e =>
      e.styleInRace === 'C' && e.finishPosition <= 3
    ).length
    if (closingWins >= 2) {
...
export function detectEquipmentRescue(
  ...
  try {
    ...
    if (trainerWinRate < 20) {
      ...
      evidenceDetails.push(`Trainer wins ${trainerWinRate}% with this change (${trainerSampleSize} starts)`)
  ...
  try {
    ...
    if (daysSinceLast && daysSinceLast >= 60) {
      ...
      if (profile && profile.overallWinRate >= 18) {
        evidenceDetails.push(`Trainer overall win rate: ${profile.overallWinRate}% (layoff specialist potential)`)
...
export function detectHiddenForm(
  ...
  try {
    ...
    if (lastRace && Math.abs(lastRace.distanceFurlongs - raceHeader.distanceFurlongs) >= 2) {
      const direction = raceHeader.distanceFurlongs > lastRace.distanceFurlongs ? 'stretching out' : 'sprinting'
