...
function validateClassDropFactor(factor: DetectedFactor): FactorValidationResult {
  ...
  const hasLevelsDrop = factor.evidenceDetails.some(
    (e) => e.toLowerCase().includes('dropping') || e.toLowerCase().includes('class level')
  );
  const hasProvenRecord = factor.evidenceDetails.some(
...
function validateEquipmentChangeFactor(factor: DetectedFactor): FactorValidationResult {
  ...
  const hasLasix = factor.evidenceDetails.some((e) => e.toLowerCase().includes('lasix'));
  const hasBlinkers = factor.evidenceDetails.some((e) => e.toLowerCase().includes('blinker'));
  const hasTrainerPattern = factor.evidenceDetails.some(
    (e) => e.toLowerCase().includes('trainer') && e.toLowerCase().includes('%')
  );
...
function validatePaceFitFactor(factor: DetectedFactor): FactorValidationResult {
  ...
  const hasRunningStyle = factor.evidenceDetails.some(
    (e) =>
      e.toLowerCase().includes('running style') ||
      e.toLowerCase().includes('closer') ||
      e.toLowerCase().includes('presser') ||
      e.toLowerCase().includes('speed')
  );
  const hasPaceScenario = factor.evidenceDetails.some(
    (e) => e.toLowerCase().includes('pace scenario') || e.toLowerCase().includes('ppi')
  );
...
function validateTrackBiasFitFactor(factor: DetectedFactor): FactorValidationResult {
  ...
  const hasTrackData = factor.evidenceDetails.some(
    (e) => e.toLowerCase().includes('track') && e.includes('%')
  );
  const hasStyleMatch = factor.evidenceDetails.some(
    (e) => e.toLowerCase().includes('this is a') || e.toLowerCase().includes('running style')
  );
...
function validateHiddenFormFactor(factor: DetectedFactor): FactorValidationResult {
  ...
  const hasWorkouts = factor.evidenceDetails.some(
    (e) => e.toLowerCase().includes('work') || e.toLowerCase().includes('bullet')
  );
  const hasExcuse = factor.evidenceDetails.some(
    (e) =>
      e.toLowerCase().includes('excuse') ||
      e.toLowerCase().includes('wide') ||
      e.toLowerCase().includes('trouble') ||
...
function validateBreedingPotentialFactor(factor: DetectedFactor): FactorValidationResult {
  ...
  const hasSireData = factor.evidenceDetails.some((e) => e.toLowerCase().includes('sire'));
...
function validateTrainerPatternFactor(factor: DetectedFactor): FactorValidationResult {
  ...
  const hasWinRate = factor.evidenceDetails.some(
    (e) =>
      e.toLowerCase().includes('win rate') ||
      (e.includes('%') && e.toLowerCase().includes('trainer'))
  );
  const hasSampleSize = factor.evidenceDetails.some(
    (e) => e.toLowerCase().includes('starts') || e.toLowerCase().includes('sample')
  );
...
export function validateDiamond(diamond: DiamondAnalysis): DiamondValidationResult {
  ...
  const adjustedConfidence = isValid ? calculateConfidence(validatedFactorCount) : 0;
 ...
 */
export function validateDiamonds(diamonds: DiamondAnalysis[]): DiamondValidationResult[] {
 ...
 */
export function getValidatedDiamonds(diamonds: DiamondAnalysis[]): DiamondAnalysis[] {
