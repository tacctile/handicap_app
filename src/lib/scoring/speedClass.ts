...
function getBestRecentFigure(
  ...
  const figures = recentPPs
    .map(extractSpeedFigure)
...
function calculateClassScore(
  ...
  if (classMovement === 'rise') {
    ...
    const wasCompetitive = lastRace.finishPosition <= 3 ||
      (lastRace.finishPosition <= 5 && lastRace.lengthsBehind < 5)
...
export function calculateSpeedClassScore(
  ...
  // Get speed figures
  const bestRecentFigure = getBestRecentFigure(horse.pastPerformances, 3) ??
    getLastBeyerFromEntry(horse)
  const averageFigure = getAverageFigure(horse.pastPerformances, 5) ??
    (horse.averageBeyer ?? null)
  ...
  const classResult = calculateClassScore(
    currentClass,
