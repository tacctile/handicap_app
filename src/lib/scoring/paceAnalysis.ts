...
function analyzeRaceRunningStyle(pp: PastPerformance): RunningStyleEvidence {
  const firstCallPosition = pp.runningLine.quarterMile ??
                           pp.runningLine.halfMile ??
                           pp.runningLine.start ??
                           pp.fieldSize

  const lengthsBehindAtFirstCall = pp.runningLine.quarterMileLengths ??
                                   pp.runningLine.halfMileLengths ??
                                   null
...
export function parseRunningStyle(horse: HorseEntry): RunningStyleProfile {
  ...
  const avgFirstCallPosition = evidence.reduce((sum, e) => sum + e.firstCallPosition, 0) / evidence.length
  const lengthsData = evidence.filter(e => e.lengthsBehindAtFirstCall !== null)
  const avgLengthsBehindEarly = lengthsData.length > 0
    ? lengthsData.reduce((sum, e) => sum + (e.lengthsBehindAtFirstCall ?? 0), 0) / lengthsData.length
    : 0
  ...
  const description = generateStyleDescription(dominantStyle, timesOnLead, timesInTop3Early, evidence.length)
