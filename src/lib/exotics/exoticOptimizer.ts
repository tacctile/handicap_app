 ...
 */
function calculateExactaProbability(firstHorses: HorseTier[], secondHorses: HorseTier[]): number {
 ...
 */
function findOptimalBaseBet(budget: number, combinations: number, maxBaseBet: number = 5): number {
 ...
 */
function generateExactaOptions(config: OptimizationConfig): OptimizedBetOption[] {
  ...
  if (tier1Horses.length >= 1 && allHorses.length > tier1Horses.length) {
    ...
    if (otherHorses.length >= 1) {
      ...
      if (cost.isValid && cost.total <= budget) {
        const allOthers = allHorses.filter(
          (h) => !tier1Horses.some((t) => t.programNumber === h.programNumber)
        );
 ...
 */
function generateTrifectaOptions(config: OptimizationConfig): OptimizedBetOption[] {
  ...
  if (tier1Horses.length >= 3) {
    ...
    if (cost.isValid && cost.total <= budget) {
      ...
      const hitProb =
        calculateTrifectaProbability(selectedHorses, selectedHorses, selectedHorses) * 0.7;
  ...
  if (tier1Horses.length >= 1 && tier1Horses.length + tier2Horses.length >= 3) {
    ...
    const testCost = calculateTrifectaPartWheelCost(
      keyHorses,
      secondThirdHorses,
      secondThirdHorses,
      1
    ...
    const baseBet = findOptimalBaseBet(budget, testCost.combinations, maxBaseBet);
    const cost = calculateTrifectaPartWheelCost(
      keyHorses,
      secondThirdHorses,
      secondThirdHorses,
      baseBet
  ...
  if (allHorses.length >= 3 && allHorses.length <= 5) {
    ...
    if (cost.isValid && cost.total <= budget) {
      ...
      const hitProb =
        calculateTrifectaProbability(selectedHorses, selectedHorses, selectedHorses) * 0.8;
      const payout = estimateExpectedPayout('trifecta', selectedHorses, baseBet);
      const ev = calculateExpectedValue(hitProb, payout.likely, cost.total);
 ...
 */
function generateSuperfectaOptions(config: OptimizationConfig): OptimizedBetOption[] {
  ...
  if (allHorses.length >= 4) {
    ...
    if (cost.isValid && cost.total <= budget) {
      const selectedHorses = allHorses.slice(0, horses.length);
      const hitProb =
        calculateSuperfectaProbability(
          selectedHorses,
          selectedHorses,
          selectedHorses,
          selectedHorses
  ...
  if (tier1Horses.length >= 1 && allHorses.length >= 4) {
    ...
    const testCost = calculateSuperfectaKeyCost(
      keyHorses,
      otherHorses,
      otherHorses,
      otherHorses,
    ...
    if (cost.isValid && cost.total <= budget) {
      const keyHorsesFull = tier1Horses.slice(0, 2);
      const hitProb = calculateSuperfectaProbability(
        keyHorsesFull,
        allHorses,
        allHorses,
        allHorses
