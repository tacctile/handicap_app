 ...
 */
function generateExactaComparisons(
  ...
  if (horses.length >= 2) {
    ...
    if (cost.isValid && cost.total <= budget) {
      ...
      const hitProb = calculateHitProbability('exacta', 'box', horses, cost.combinations, fieldSize)
  ...
  if (horses.length >= 2) {
    ...
    if (cost.isValid && cost.total <= budget) {
      const payoutRange = quickPayoutEstimate('exacta', odds, baseBet)
      const hitProb = calculateHitProbability('exacta', 'key_over', horses, cost.combinations, fieldSize)
      const ev = calculateExpectedValue(hitProb, payoutRange.likely, cost.total)
  ...
  if (horses.length >= 1 && fieldSize > horses.length) {
    ...
    if (cost.isValid && cost.total <= budget) {
      const payoutRange = quickPayoutEstimate('exacta', odds, baseBet)
      const hitProb = calculateHitProbability('exacta', 'wheel', horses, cost.combinations, fieldSize)
 ...
 */
function generateTrifectaComparisons(
  config: ComparisonConfig,
  baseBet: number
): ComparisonRow[] {
  const { budget, horses, fieldSize } = config
  ...
  if (horses.length >= 3) {
    ...
    if (cost.isValid && cost.total <= budget) {
      ...
      const hitProb = calculateHitProbability('trifecta', 'box', horses, cost.combinations, fieldSize)
      const ev = calculateExpectedValue(hitProb, payoutRange.likely, cost.total)
  ...
  if (horses.length >= 3) {
    ...
    if (cost.isValid && cost.total <= budget) {
      ...
      const hitProb = calculateHitProbability('trifecta', 'key_over', horses, cost.combinations, fieldSize)
      const ev = calculateExpectedValue(hitProb, payoutRange.likely, cost.total)
  ...
  if (horses.length >= 3) {
    ...
    if (cost.isValid && cost.total <= budget) {
      const payoutRange = quickPayoutEstimate('trifecta', odds, baseBet)
      const hitProb = calculateHitProbability('trifecta', 'part_wheel', horses, cost.combinations, fieldSize)
      const ev = calculateExpectedValue(hitProb, payoutRange.likely, cost.total)
  ...
  if (horses.length >= 1 && fieldSize >= 3) {
    ...
    if (cost.isValid && cost.total <= budget) {
      const payoutRange = quickPayoutEstimate('trifecta', odds, baseBet)
      const hitProb = calculateHitProbability('trifecta', 'wheel', horses, cost.combinations, fieldSize)
 ...
 */
function generateSuperfectaComparisons(
  config: ComparisonConfig,
  baseBet: number
): ComparisonRow[] {
  const { budget, horses, fieldSize } = config
  ...
  if (horses.length >= 4) {
    ...
    if (cost.isValid && cost.total <= budget) {
      ...
      const hitProb = calculateHitProbability('superfecta', 'box', horses, cost.combinations, fieldSize)
      const ev = calculateExpectedValue(hitProb, payoutRange.likely, cost.total)
  ...
  if (horses.length >= 4) {
    const keyHorse = [horses[0].programNumber]
    const otherHorses = horses.slice(1).map(h => h.programNumber)
    const cost = calculateSuperfectaKeyCost(keyHorse, otherHorses, otherHorses, otherHorses, baseBet)

    if (cost.isValid && cost.total <= budget) {
      const payoutRange = quickPayoutEstimate('superfecta', odds, baseBet)
      const hitProb = calculateHitProbability('superfecta', 'key_over', horses, cost.combinations, fieldSize)
      const ev = calculateExpectedValue(hitProb, payoutRange.likely, cost.total)
  ...
  if (horses.length >= 4) {
    const firstHorses = horses.slice(0, 2).map(h => h.programNumber)
    const otherHorses = horses.map(h => h.programNumber)
    const cost = calculateSuperfectaPartWheelCost(firstHorses, otherHorses, otherHorses, otherHorses, baseBet)

    if (cost.isValid && cost.total <= budget) {
      const payoutRange = quickPayoutEstimate('superfecta', odds, baseBet)
      const hitProb = calculateHitProbability('superfecta', 'part_wheel', horses, cost.combinations, fieldSize)
      const ev = calculateExpectedValue(hitProb, payoutRange.likely, cost.total)
