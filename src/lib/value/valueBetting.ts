...
import { analyzeMarketInefficiency, type MarketInefficiencyAnalysis } from './marketInefficiency';
 ...
 */
function calculateKellyFraction(winProbability: number, decimalOdds: number, edge: number): number {
 ...
 */
function optimizeBetAllocation(bets: ValueBet[], config: ValueStrategyConfig): ValueBet[] {
...
export function generateValueBettingPlan(
  ...
  const avoidBets = allAnalyzed.filter(
    (bet) =>
  ...
  const expectedProfit = recommendedBets.reduce(
    (sum, b) => sum + b.evPerDollar * b.recommendedAmount,
    0
  );
  const expectedROI =
    totalRecommendedWager > 0 ? (expectedProfit / totalRecommendedWager) * 100 : 0;
...
function generateStrategyExplanation(
  ...
  switch (mode) {
    case 'pure_value':
      explanations.push(
        'Pure Value mode: Betting any positive expected value, regardless of confidence.'
      );
      break;
    case 'balanced_value':
      explanations.push(
        'Balanced Value mode: Positive EV bets with reasonable confidence threshold.'
      );
      break;
    case 'conservative_value':
      explanations.push(
        'Conservative Value mode: Only strong value (25%+ EV) with high confidence.'
      );
      break;
  }
  ...
  if (bets.length === 0) {
    explanations.push(
      'No bets meet the value threshold for this race. Consider passing or waiting for better spots.'
    );
    return explanations;
  }
  ...
  if (stats.longshotCount > 0) {
    explanations.push(
      `${stats.longshotCount} positive EV longshot(s) included - good value despite lower probability.`
    );
  }
...
function generateWarnings(
  ...
  if (favoriteAvoid) {
    warnings.push(
      `Favorite (#${favoriteAvoid.programNumber}) is negative EV. Public may be overbet.`
    );
  }
