...
describe('DEFAULT_TIERS', () => {
  it('should have correct tier probabilities', () => {
    ...
    const strongTier = DEFAULT_TIERS.find(t => t.minScore === 180)
    expect(strongTier?.winProbability).toBe(65)

    const poorTier = DEFAULT_TIERS.find(t => t.maxScore <= 100)
...
describe('calculateBrierScore', () => {
  ...
  it('should return higher score for worse predictions', () => {
    ...
    expect(calculateBrierScore(badPredictions)).toBeGreaterThan(calculateBrierScore(goodPredictions))
  })
...
describe('generateCalibrationSummary', () => {
  ...
  it('should suggest more samples when insufficient data', () => {
    ...
    const needsMoreSamples = summary.suggestions.some(s =>
      s.toLowerCase().includes('need') && s.toLowerCase().includes('more')
    )
