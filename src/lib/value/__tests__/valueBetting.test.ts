...
describe('generateValueBettingPlan', () => {
  it('should generate a betting plan for a race', () => {
    const horses = [
      { horse: createMockHorse({ programNumber: 1, morningLineOdds: '10-1' }), score: createMockScore(180) },
      { horse: createMockHorse({ programNumber: 2, morningLineOdds: '5-1' }), score: createMockScore(160) },
      { horse: createMockHorse({ programNumber: 3, morningLineOdds: '3-1' }), score: createMockScore(140) },
    ]
  ...
  it('should rank bets by EV', () => {
    const horses = [
      { horse: createMockHorse({ programNumber: 1, morningLineOdds: '20-1' }), score: createMockScore(175) },
      { horse: createMockHorse({ programNumber: 2, morningLineOdds: '3-1' }), score: createMockScore(180) },
      { horse: createMockHorse({ programNumber: 3, morningLineOdds: '10-1' }), score: createMockScore(170) },
    ]
    const raceHeader = createMockRaceHeader()
  ...
  it('should identify best bet', () => {
    const horses = [
      { horse: createMockHorse({ programNumber: 1, morningLineOdds: '15-1' }), score: createMockScore(185) },
      { horse: createMockHorse({ programNumber: 2, morningLineOdds: '5-1' }), score: createMockScore(160) },
    ]
    const raceHeader = createMockRaceHeader()
...
describe('Strategy Presets', () => {
  ...
  it('conservative mode should be most selective', () => {
    ...
    expect(conservative.recommendedBets.length).toBeLessThanOrEqual(balanced.recommendedBets.length)
...
describe('getValueBetColor', () => {
  it('should return color based on classification', () => {
    const eliteBet = { classification: 'elite_value' as const } as Parameters<typeof getValueBetColor>[0]
    const strongBet = { classification: 'strong_value' as const } as Parameters<typeof getValueBetColor>[0]
    const negativeBet = { classification: 'negative_value' as const } as Parameters<typeof getValueBetColor>[0]
