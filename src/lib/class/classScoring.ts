...
function scoreClassMovement(
  ...
  if (movement.direction === 'drop') {
    ...
    if (movement.claimingPriceDrop && movement.claimingPriceDrop >= 10000) {
      const claimBonus = Math.min(
        SCORE_WEIGHTS.CLAIMING_DROP_10K_PLUS,
        Math.floor(movement.claimingPriceDrop / 5000)
      );
      score += claimBonus;
      breakdown.push({
    ...
    if (
      (movement.fromLevel === ClassLevel.MAIDEN_SPECIAL_WEIGHT ||
        movement.fromLevel === ClassLevel.MAIDEN_CLAIMING) &&
 ...
 */
export function calculateClassScore(horse: HorseEntry, raceHeader: RaceHeader): ClassScoreResult {
  // Get full analysis
  ...
  // Calculate total
  let total = provenResult.score + movementResult.score + hiddenResult.score + trackTierScore;
  ...
  const reasoning =
    reasoningParts.length > 0 ? reasoningParts.join(' | ') : 'Class analysis neutral';
