...
describe('Full Flow Integration', () => {
  ...
  describe('Scratch Horse → Recalculate Flow', () => {
    it('removes scratched horse from rankings', () => {
      ...
      const initialScored = calculateRaceScores(
        horses,
        header,
        getOdds,
        () => false,
      ...
      const rescored = calculateRaceScores(
        horses,
        header,
        getOdds,
        isScratched,
      ...
      const scratchedResult = rescored.find(sh => sh.horse.horseName === topHorse.horse.horseName)
  ...
  describe('Track Condition Changes', () => {
    it('recalculates when track condition changes from fast to sloppy', () => {
      const horse = createHorseEntry({
        ...
        wetWins: 2,
        pastPerformances: [
          createPastPerformance({ trackCondition: 'sloppy', finishPosition: 1 }),
        ],
      })
      const header = createRaceHeader()
