/**
 * Algorithm Validation Test
 *
 * Compares BASE ABILITY (base score) rankings against actual race results
 * to measure algorithm accuracy.
 *
 * USAGE:
 *   npm test -- --run src/__tests__/validation/algorithmVsResults.test.ts
 *
 * FILE NAMING CONVENTION:
 *   DRF files:     {CODE}{MMDD}.DRF        (e.g., SAX1229.DRF, GPX0102.DRF)
 *   Results files: {CODE}{MMDD}_results.txt (e.g., SAX1229_results.txt, GPX0102_results.txt)
 *
 * Both files must be in src/data/ folder. The test auto-discovers all matching pairs.
 */

import { describe, it, expect } from 'vitest';
import * as fs from 'fs';
import * as path from 'path';

import { parseDRFFile } from '../../lib/drfParser';
import { calculateRaceScores } from '../../lib/scoring';
import type { TrackCondition } from '../../hooks/useRaceState';

// ============================================================================
// CONFIGURATION
// ============================================================================

const DATA_DIR = path.join(__dirname, '../../data');

// ============================================================================
// TYPES
// ============================================================================

interface ActualResult {
  raceNumber: number;
  finishers: { position: number; postPosition: number; horseName: string }[];
  scratches: string[];
}

interface RaceAnalysis {
  raceNumber: number;
  raceInfo: string;
  scratches: string[];
  ourTop4: { post: number; name: string; baseScore: number }[];
  actualTop4: { position: number; postPosition: number; horseName: string }[];
  fullField: {
    rank: number;
    post: number;
    name: string;
    baseScore: number;
    actualFinish: string;
  }[];
  winnerHit: boolean;
  winnerInTop2: boolean;
  winnerInTop3: boolean;
  exactaHit: boolean;
  trifectaHit: boolean;
}

interface FileSetResults {
  filePrefix: string;
  trackCode: string;
  trackName: string;
  raceDate: string;
  totalRaces: number;
  winnersCorrect: number;
  top2Correct: number;
  top3Correct: number;
  exactaCorrect: number;
  trifectaCorrect: number;
  superfectaCorrect: number;
  raceAnalyses: RaceAnalysis[];
}

// ============================================================================
// FILE DISCOVERY
// ============================================================================

function discoverFilePairs(): { drfPath: string; resultsPath: string; prefix: string }[] {
  const pairs: { drfPath: string; resultsPath: string; prefix: string }[] = [];

  if (!fs.existsSync(DATA_DIR)) {
    return pairs;
  }

  const files = fs.readdirSync(DATA_DIR);
  const drfFiles = files.filter((f) => f.toUpperCase().endsWith('.DRF'));

  for (const drfFile of drfFiles) {
    // Extract prefix (e.g., "SAX1229" from "SAX1229.DRF")
    const prefix = drfFile.replace(/\.DRF$/i, '');
    const resultsFile = `${prefix}_results.txt`;

    if (files.includes(resultsFile)) {
      pairs.push({
        drfPath: path.join(DATA_DIR, drfFile),
        resultsPath: path.join(DATA_DIR, resultsFile),
        prefix,
      });
    }
  }

  // Sort by prefix for consistent output
  pairs.sort((a, b) => a.prefix.localeCompare(b.prefix));

  return pairs;
}

// ============================================================================
// PARSE ACTUAL RESULTS FILE
// ============================================================================

function parseResultsFile(content: string): ActualResult[] {
  const results: ActualResult[] = [];
  const lines = content.split('\n');

  let currentRace: ActualResult | null = null;

  for (const line of lines) {
    const trimmed = line.replace(/^\s*\d+→/, '').trim();

    const raceMatch = trimmed.match(/^RACE\s+(\d+)/i);
    if (raceMatch) {
      if (currentRace) {
        results.push(currentRace);
      }
      currentRace = {
        raceNumber: parseInt(raceMatch[1]),
        finishers: [],
        scratches: [],
      };
      continue;
    }

    if (!currentRace) continue;

    const finishMatch = trimmed.match(/^(1st|2nd|3rd|4th):\s*(\d+)\s+(.+)/i);
    if (finishMatch) {
      const posMap: Record<string, number> = { '1st': 1, '2nd': 2, '3rd': 3, '4th': 4 };
      currentRace.finishers.push({
        position: posMap[finishMatch[1].toLowerCase()],
        postPosition: parseInt(finishMatch[2]),
        horseName: finishMatch[3].trim(),
      });
      continue;
    }

    const scratchMatch = trimmed.match(/^SCRATCHED:\s*(.+)/i);
    if (scratchMatch) {
      const scratchText = scratchMatch[1].trim();
      if (scratchText.toLowerCase() !== 'none') {
        const scratchParts = scratchText.split(',');
        for (const part of scratchParts) {
          const match = part.trim().match(/^\d+\s+(.+)/);
          if (match) {
            currentRace.scratches.push(match[1].trim().toLowerCase());
          }
        }
      }
    }
  }

  if (currentRace) {
    results.push(currentRace);
  }

  return results;
}

function getOrdinal(n: number): string {
  const s = ['th', 'st', 'nd', 'rd'];
  const v = n % 100;
  return s[(v - 20) % 10] || s[v] || s[0];
}

// ============================================================================
// PROCESS SINGLE FILE SET
// ============================================================================

function processFileSet(
  drfPath: string,
  resultsPath: string,
  prefix: string,
  _verbose: boolean = true
): FileSetResults {
  const drfContent = fs.readFileSync(drfPath, 'utf-8');
  const resultsContent = fs.readFileSync(resultsPath, 'utf-8');

  const parseResult = parseDRFFile(drfContent);
  const actualResults = parseResultsFile(resultsContent);

  // Extract track info from first race's header
  const firstRace = parseResult.races[0];
  const trackCode = firstRace?.header.trackCode || prefix.substring(0, 2);
  const raceDate = firstRace?.header.raceDate || 'Unknown';

  // Statistics
  let totalRaces = 0;
  let winnersCorrect = 0;
  let top2Correct = 0;
  let top3Correct = 0;
  let exactaCorrect = 0;
  let trifectaCorrect = 0;
  let superfectaCorrect = 0;

  const raceAnalyses: RaceAnalysis[] = [];

  // Process each race
  for (const race of parseResult.races) {
    const actual = actualResults.find((r) => r.raceNumber === race.header.raceNumber);
    if (!actual) continue;

    totalRaces++;

    const scratchedNames = new Set(actual.scratches.map((s) => s.toLowerCase()));

    const trackCondition: TrackCondition = {
      surface: race.header.surface,
      condition: race.header.condition || 'fast',
      variant: 0,
    };

    const getOdds = (index: number) => {
      return race.horses[index]?.morningLineOdds || '5-1';
    };

    const isScratched = (index: number) => {
      const horse = race.horses[index];
      if (!horse) return false;
      return scratchedNames.has(horse.horseName.toLowerCase());
    };

    const scoredHorses = calculateRaceScores(
      race.horses,
      race.header,
      getOdds,
      isScratched,
      trackCondition
    );

    const activeHorses = scoredHorses
      .filter((h) => !scratchedNames.has(h.horse.horseName.toLowerCase()))
      .sort((a, b) => b.score.baseScore - a.score.baseScore);

    const ourTop4 = activeHorses.slice(0, 4).map((h) => ({
      post: h.horse.programNumber,
      name: h.horse.horseName,
      baseScore: h.score.baseScore,
    }));

    const actualTop4 = actual.finishers.slice(0, 4);
    const actualWinner = actualTop4[0];

    const winnerHit = ourTop4[0]?.post === actualWinner?.postPosition;
    const winnerInTop2 = ourTop4.slice(0, 2).some((h) => h.post === actualWinner?.postPosition);
    const winnerInTop3 = ourTop4.slice(0, 3).some((h) => h.post === actualWinner?.postPosition);
    const exactaHit =
      ourTop4[0]?.post === actualTop4[0]?.postPosition &&
      ourTop4[1]?.post === actualTop4[1]?.postPosition;
    const trifectaHit = exactaHit && ourTop4[2]?.post === actualTop4[2]?.postPosition;
    const superfectaHit = trifectaHit && ourTop4[3]?.post === actualTop4[3]?.postPosition;

    if (winnerHit) winnersCorrect++;
    if (winnerInTop2) top2Correct++;
    if (winnerInTop3) top3Correct++;
    if (exactaHit) exactaCorrect++;
    if (trifectaHit) trifectaCorrect++;
    if (superfectaHit) superfectaCorrect++;

    // Build full field analysis
    const fullField = activeHorses.map((h, i) => {
      const actualFinish = actual.finishers.find((f) => f.postPosition === h.horse.programNumber);
      return {
        rank: i + 1,
        post: h.horse.programNumber,
        name: h.horse.horseName,
        baseScore: h.score.baseScore,
        actualFinish: actualFinish
          ? `${actualFinish.position}${getOrdinal(actualFinish.position)}`
          : '---',
      };
    });

    raceAnalyses.push({
      raceNumber: race.header.raceNumber,
      raceInfo: `${race.header.distance} ${race.header.surface} - ${race.header.raceConditions || race.header.raceType}`,
      scratches: actual.scratches,
      ourTop4,
      actualTop4,
      fullField,
      winnerHit,
      winnerInTop2,
      winnerInTop3,
      exactaHit,
      trifectaHit,
    });
  }

  return {
    filePrefix: prefix,
    trackCode: trackCode,
    trackName: trackCode, // Use track code as name (could lookup from trackDatabase)
    raceDate: raceDate,
    totalRaces,
    winnersCorrect,
    top2Correct,
    top3Correct,
    exactaCorrect,
    trifectaCorrect,
    superfectaCorrect,
    raceAnalyses,
  };
}

// ============================================================================
// PRINT FUNCTIONS
// ============================================================================

function printRaceDetails(analysis: RaceAnalysis, trackInfo: string): void {
  console.log(`\n${'═'.repeat(80)}`);
  console.log(`${trackInfo} - RACE ${analysis.raceNumber}: ${analysis.raceInfo}`);
  console.log(`${'═'.repeat(80)}\n`);

  if (analysis.scratches.length > 0) {
    console.log(`🚫 SCRATCHES: ${analysis.scratches.join(', ')}\n`);
  }

  console.log(`┌─────────────────────────────────────────────────────────────────────────────┐`);
  console.log(`│  OUR RANKING (by Base Score)          │  ACTUAL FINISH ORDER               │`);
  console.log(`├─────────────────────────────────────────────────────────────────────────────┤`);

  for (let i = 0; i < 4; i++) {
    const ours = analysis.ourTop4[i];
    const theirs = analysis.actualTop4[i];

    const ourStr = ours
      ? `${i + 1}. #${ours.post.toString().padEnd(2)} ${ours.name.substring(0, 18).padEnd(18)} (${ours.baseScore})`
      : `${i + 1}. ---`;

    const theirStr = theirs
      ? `${i + 1}. #${theirs.postPosition.toString().padEnd(2)} ${theirs.horseName.substring(0, 18).padEnd(18)}`
      : `${i + 1}. ---`;

    const match = ours && theirs && ours.post === theirs.postPosition;
    const marker = match ? ' ✓' : '  ';

    console.log(`│  ${ourStr.padEnd(36)} │  ${theirStr.padEnd(32)}${marker} │`);
  }

  console.log(`└─────────────────────────────────────────────────────────────────────────────┘\n`);

  // Full field table
  console.log(`FULL FIELD (sorted by Base Score):`);
  console.log(`${'─'.repeat(70)}`);
  console.log(`Rank  PP  Horse Name                   Base   Actual Finish`);
  console.log(`${'─'.repeat(70)}`);

  for (const horse of analysis.fullField) {
    const finishNum = parseInt(horse.actualFinish);
    const highlight =
      finishNum === 1 ? ' ← WINNER' : finishNum === 2 ? ' ← 2nd' : finishNum === 3 ? ' ← 3rd' : '';

    console.log(
      `${horse.rank.toString().padStart(3)}.  ${horse.post.toString().padStart(2)}  ${horse.name.substring(0, 25).padEnd(25)}  ${horse.baseScore.toString().padStart(3)}    ${horse.actualFinish.padStart(4)}${highlight}`
    );
  }

  console.log();
  if (analysis.winnerHit) {
    console.log(`✅ WINNER CORRECT! Our #1 pick won.`);
  } else if (analysis.winnerInTop2) {
    console.log(`🟡 Winner was our #2 pick.`);
  } else if (analysis.winnerInTop3) {
    console.log(`🟠 Winner was our #3 pick.`);
  } else {
    const actualWinner = analysis.actualTop4[0];
    const winnerRank =
      analysis.fullField.find((h) => h.post === actualWinner?.postPosition)?.rank || '?';
    console.log(
      `❌ MISSED. Winner (#${actualWinner?.postPosition} ${actualWinner?.horseName}) was our #${winnerRank} pick.`
    );
  }
}

function printTrackSummary(results: FileSetResults): void {
  const pct = (n: number) => ((n / results.totalRaces) * 100).toFixed(1);

  console.log(`\n┌──────────────────────────────────────────────────────────────┐`);
  console.log(
    `│  ${results.trackName.padEnd(20)} (${results.trackCode}) - ${results.raceDate}`.padEnd(62) +
      `│`
  );
  console.log(`├──────────────────────────────────────────────────────────────┤`);
  console.log(
    `│  Races: ${results.totalRaces.toString().padEnd(3)}  │  Win: ${pct(results.winnersCorrect).padStart(5)}%  │  Top2: ${pct(results.top2Correct).padStart(5)}%  │  Top3: ${pct(results.top3Correct).padStart(5)}%  │`
  );
  console.log(`└──────────────────────────────────────────────────────────────┘`);
}

// ============================================================================
// MAIN TEST
// ============================================================================

describe('Algorithm Validation: BASE ABILITY vs Actual Results', () => {
  it('validates algorithm predictions against actual race results', () => {
    const filePairs = discoverFilePairs();

    if (filePairs.length === 0) {
      console.log('\n⚠️  No matching DRF/results file pairs found in src/data/');
      console.log('    Expected format: {CODE}{MMDD}.DRF + {CODE}{MMDD}_results.txt');
      console.log('    Example: SAX1229.DRF + SAX1229_results.txt\n');
      expect(true).toBe(true); // Pass test but warn
      return;
    }

    // Print header
    console.log('\n');
    console.log('╔══════════════════════════════════════════════════════════════════════════════╗');
    console.log('║          ALGORITHM VALIDATION: BASE ABILITY vs ACTUAL RESULTS               ║');
    console.log(
      '╚══════════════════════════════════════════════════════════════════════════════╝\n'
    );

    console.log(`📁 Found ${filePairs.length} file pair(s) to process:\n`);
    for (const pair of filePairs) {
      console.log(`   • ${pair.prefix}`);
    }
    console.log();

    // Process all file pairs
    const allResults: FileSetResults[] = [];

    for (const pair of filePairs) {
      const results = processFileSet(pair.drfPath, pair.resultsPath, pair.prefix, true);
      allResults.push(results);

      // Print race-by-race details
      const trackInfo = `${results.trackCode} ${results.raceDate}`;
      for (const analysis of results.raceAnalyses) {
        printRaceDetails(analysis, trackInfo);
      }
    }

    // Aggregate totals
    const totals = {
      races: 0,
      winners: 0,
      top2: 0,
      top3: 0,
      exacta: 0,
      trifecta: 0,
      superfecta: 0,
    };

    for (const r of allResults) {
      totals.races += r.totalRaces;
      totals.winners += r.winnersCorrect;
      totals.top2 += r.top2Correct;
      totals.top3 += r.top3Correct;
      totals.exacta += r.exactaCorrect;
      totals.trifecta += r.trifectaCorrect;
      totals.superfecta += r.superfectaCorrect;
    }

    // Print per-track summaries
    console.log('\n\n' + '█'.repeat(80));
    console.log('                           PER-TRACK BREAKDOWN');
    console.log('█'.repeat(80));

    for (const results of allResults) {
      printTrackSummary(results);
    }

    // Print overall summary
    console.log('\n\n' + '█'.repeat(80));
    console.log('                         OVERALL ALGORITHM PERFORMANCE');
    console.log('█'.repeat(80) + '\n');

    console.log(`Total File Pairs: ${allResults.length}`);
    console.log(`Total Races Analyzed: ${totals.races}\n`);

    const pct = (n: number) => ((n / totals.races) * 100).toFixed(1);

    console.log('┌──────────────────────────────────────────────────────────────┐');
    console.log('│  METRIC                          │  COUNT  │  PERCENTAGE    │');
    console.log('├──────────────────────────────────────────────────────────────┤');
    console.log(
      `│  Winners Picked Correctly (#1)   │   ${totals.winners.toString().padStart(3)}   │     ${pct(totals.winners).padStart(5)}%     │`
    );
    console.log(
      `│  Winner in Our Top 2             │   ${totals.top2.toString().padStart(3)}   │     ${pct(totals.top2).padStart(5)}%     │`
    );
    console.log(
      `│  Winner in Our Top 3             │   ${totals.top3.toString().padStart(3)}   │     ${pct(totals.top3).padStart(5)}%     │`
    );
    console.log('├──────────────────────────────────────────────────────────────┤');
    console.log(
      `│  Exacta (1-2 in order)           │   ${totals.exacta.toString().padStart(3)}   │     ${pct(totals.exacta).padStart(5)}%     │`
    );
    console.log(
      `│  Trifecta (1-2-3 in order)       │   ${totals.trifecta.toString().padStart(3)}   │     ${pct(totals.trifecta).padStart(5)}%     │`
    );
    console.log(
      `│  Superfecta (1-2-3-4 in order)   │   ${totals.superfecta.toString().padStart(3)}   │     ${pct(totals.superfecta).padStart(5)}%     │`
    );
    console.log('└──────────────────────────────────────────────────────────────┘');

    // Assessment
    console.log('\n' + '─'.repeat(80));
    console.log('ASSESSMENT:');
    console.log('─'.repeat(80));

    const winPct = (totals.winners / totals.races) * 100;
    const top3Pct = (totals.top3 / totals.races) * 100;

    if (winPct >= 33) {
      console.log('🏆 EXCELLENT: Picking winners at 33%+ is profitable territory!');
    } else if (winPct >= 25) {
      console.log('✅ GOOD: Solid win rate, competitive with typical handicapping.');
    } else if (winPct >= 15) {
      console.log('🟡 FAIR: Below average win rate, algorithm needs refinement.');
    } else {
      console.log('❌ POOR: Significant algorithm issues - investigate scoring weights.');
    }

    if (top3Pct >= 66) {
      console.log('🎯 Winner appearing in top 3 picks 66%+ of the time is excellent!');
    } else if (top3Pct >= 50) {
      console.log('👍 Winner in top 3 at least 50% - decent predictive power.');
    } else {
      console.log('⚠️  Winner outside top 3 too often - consider algorithm adjustments.');
    }

    // Sample size assessment
    console.log('\n' + '─'.repeat(80));
    console.log('SAMPLE SIZE CONFIDENCE:');
    console.log('─'.repeat(80));

    if (totals.races >= 300) {
      console.log(`📊 ${totals.races} races = HIGH confidence (±5% margin of error)`);
    } else if (totals.races >= 100) {
      console.log(`📊 ${totals.races} races = MODERATE confidence (±10% margin of error)`);
    } else if (totals.races >= 50) {
      console.log(`📊 ${totals.races} races = LOW confidence (±14% margin of error)`);
    } else {
      console.log(`📊 ${totals.races} races = VERY LOW confidence (±30%+ margin of error)`);
      console.log('   → Need at least 100+ races for meaningful analysis');
    }

    console.log('\n' + '═'.repeat(80) + '\n');

    // The test always passes - this is for output/validation purposes
    expect(totals.races).toBeGreaterThan(0);
  });
});
