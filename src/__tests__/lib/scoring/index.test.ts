...
describe('Main Scoring Engine', () => {
  ...
  describe('SCORE_LIMITS Enforcement', () => {
    it('total score is capped at MAX_SCORE (240)', () => {
  ...
  describe('Determinism', () => {
    it('same inputs produce same outputs', () => {
      const horse = createHorseEntry({
        ...
        postPosition: 5,
        pastPerformances: [
          createPastPerformance({ finishPosition: 2, speedFigures: createSpeedFigures({ beyer: 85 }) }),
          createPastPerformance({ finishPosition: 3, speedFigures: createSpeedFigures({ beyer: 82 }) }),
        ],
      })
      const header = createRaceHeader()
  ...
  describe('calculateRaceConfidence', () => {
    ...
    it('higher confidence for larger score separation', () => {
      ...
      const horse1 = createHorseEntry({
        postPosition: 4,
        pastPerformances: Array.from({ length: 5 }, () =>
          createPastPerformance({ finishPosition: 1, speedFigures: createSpeedFigures({ beyer: 95 }) })
        ),
      })
      ...
      const scoredHorses = calculateRaceScores([horse1, horse2], header, getOdds, isScratched, 'fast')
      const confidence = calculateRaceConfidence(scoredHorses)
