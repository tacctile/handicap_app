...
export function useSubscription(): UseSubscriptionReturn {
  ...
  const [subscriptionService] = useState<ISubscriptionService>(() => getSubscriptionService());
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscriptionService.onSubscriptionChange(user.id, (newSubscription) => {
  ...
  const openCheckout = useCallback(
    async (planId?: string): Promise<string> => {
      if (!user) {
        const err = createSubscriptionError('NOT_AUTHENTICATED', 'Must be signed in to subscribe');
        setError(err);
        throw err;
      }
  ...
  const cancel = useCallback(async (): Promise<void> => {
    if (!subscription) {
      const err = createSubscriptionError('SUBSCRIPTION_NOT_FOUND', 'No subscription to cancel');
      setError(err);
      throw err;
    }
  ...
  const resume = useCallback(async (): Promise<void> => {
    if (!subscription) {
      const err = createSubscriptionError('SUBSCRIPTION_NOT_FOUND', 'No subscription to resume');
      setError(err);
      throw err;
    }

    if (!subscriptionService.resumeSubscription) {
      const err = createSubscriptionError('UNKNOWN_ERROR', 'Resume not supported by this provider');
      setError(err);
      throw err;
    }
 ...
 */
export function setMockSubscriptionStatus(userId: string, status: SubscriptionStatus): void {
