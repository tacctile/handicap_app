...
export function formatHorseForPrompt(score: HorseScoreForAI): string {
  ...
  lines.push(
    `Algorithm Rank: ${score.rank} | Score: ${score.finalScore}/376 | Tier: ${score.confidenceTier}`
  );
  lines.push(
...
You are an expert horse racing handicapper working alongside an algorithm. The algorithm scored each horse using 336 data points across 15 categories (see reference above). You add the human element — reading between the lines, catching what pure math misses.
...
export function formatFavoriteForAnalysis(
  ...
  lines.push(
    `Algorithm Rank: ${favorite.rank} | Score: ${favorite.finalScore}/376 | Tier: ${favorite.confidenceTier}`
  );
